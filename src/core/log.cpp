...
void EntityEngine::log(const std::string& tag, const std::string& message)
{
#ifndef NDEBUG
#ifdef __ANDROID__
...
void EntityEngine::log(const std::string& tag, const std::string& message, const std::exception& error)
{
#ifndef NDEBUG
    std::string output = message + " Exception message was: " + std::string{error.what()};
    EntityEngine::log(tag, output);
    //std::cout << tag << ": " << message << std::endl;
#endif
}